# 🎓 CareerVerse — Project Defense & Presentation Guide
...
To impress your project reviewers, each team member should present a distinct architectural layer and be ready to answer specialized questions.
...
| Technology | Purpose | Why We Chose It (Justification for Reviewers) |
| :--- | :--- | :--- |
| **Next.js 15 (App Router)** | Full-Stack Web Framework | Enables Server-Side Rendering (SSR) for fast initial loads combined with fast client-side navigation. |
| **TypeScript** | Type-Safe Language | Prevents runtime bugs (`TypeError`, `Undefined` errors) across our complex game state and AI payloads. |
| **Groq AI (Llama 3.3 70B)** | AI Inference Engine | Ultra-fast inference speed (500+ tokens/sec) allowing instantaneous AI question generation without user delay. |
| **Framer Motion** | Animation Library | Smooth micro-animations for card transitions, typewriter dialogue, and battle feedback drawers. |
| **HTML5 Canvas API** | Client Image Processing | Dynamically strips white/grey backgrounds from character PNGs directly in browser memory without manual editing. |
| **Tailwind CSS v4 + HSL Tokens** | Design System & Utility CSS | Custom HSL color variables (`--teal`, `--purple`, `--emerald`, `--amber`, `--rose`, `--blue`) for glowing aesthetic cards. |
| **Better Auth / LocalStorage** | Authentication & Persistence | Hybrid storage model supporting instant client session persistence with server-side auth support. |
...
1. **User Landing & Onboarding**:
   - The student enters their name.
   - Groq AI generates 6 personalized questions focusing on education background, coding experience, problem-solving preferences, IT activities, and learning styles.
...
## ❓ 4. Expected Reviewer Questions & Model Answers (Viva Defense Script)

### Q1: Why did you use Generative AI instead of a pre-built static database of questions?
> **Answer:** *"Static databases offer the same fixed questions to every student, which leads to memorization rather than true learning. By integrating Groq Llama 3.3 70B AI, our platform generates personalized questions based on the user's specific background, interests, and live capacity level. Furthermore, if a student gets a question wrong, the AI dynamically creates a simpler follow-up question specifically targeting that exact concept."*
...
## 🏆 5. Demo Script for Reviewers (3-Minute Presentation Walkthrough)
